...
async function example1_BasicTOCWithLogging() {
  ...
  // Add title
  doc.createParagraph('Document with Table of Contents')
    .setStyle('Title');
...
async function example3_TOCWithCustomStyling() {
  ...
  // Custom title
  doc.createParagraph('Advanced Document')
    .setStyle('Title')
    .setAlignment('center');
  ...
  const toc = TableOfContents.create({
    ...
    useHyperlinks: true,
    headingLevelRange: { start: 1, end: 3 }, // Include levels 1-3
  });
  doc.addTableOfContents(toc);
...
async function example4_MultiLevelTOC() {
  ...
  const toc = TableOfContents.create({
    ...
    showPageNumbers: true,
    useHyperlinks: true,
    headingLevelRange: { start: 1, end: 4 },
  });
  doc.addTableOfContents(toc);
...
async function example5_TOCUpdateWorkflow() {
  ...
  console.log('Or manually: Click TOC → Press F9 → Select "Update entire table"');
}
...
async function example6_TOCBestPractices() {
  ...
  // 1. Clear title
  doc.createParagraph('Professional Report')
    .setStyle('Title')
    .setAlignment('center');
  ...
  const toc = TableOfContents.create({
    ...
    rightAlignPageNumbers: true,
    useHyperlinks: true,
    headingLevelRange: { start: 1, end: 3 }, // Don't go too deep
  });
  doc.addTableOfContents(toc);
