 ...
 */
async function createDocWithShadingXml(
  shdXml: string,
  elementType: 'paragraph' | 'run' | 'table' | 'cell' | 'tblPrEx' = 'paragraph'
): Promise<Buffer> {
  ...
  if (elementType === 'paragraph') {
    ...
    modifiedXml = docXml.replace('<w:rPr>', `<w:rPr>${shdXml}`);
    // If no rPr exists, add one
    if (modifiedXml === docXml) {
      modifiedXml = docXml.replace('<w:t', `<w:rPr>${shdXml}</w:rPr><w:t`);
    }
...
const THEME_SHD =
  '<w:shd w:val="clear" w:fill="F2F2F2" w:color="auto" w:themeFill="background1" w:themeFillShade="F2" w:themeColor="text1" w:themeTint="0D" w:themeFillTint="AA" w:themeShade="BB"/>';
