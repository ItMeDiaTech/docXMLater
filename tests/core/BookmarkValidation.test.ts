...
describe('Bookmark Pair Validation', () => {
  describe('validateBookmarkPairs()', () => {
    it('should add missing bookmarkEnd for orphaned bookmarkStart', () => {
      ...
      para1.addBookmarkStart(
        new Bookmark({ id: 10, name: 'orphaned_start', skipNormalization: true })
      );
    ...
    it('should scan bookmarks inside table cells', () => {
      ...
      if (cell) {
        ...
        cellPara.addBookmarkStart(
          new Bookmark({ id: 20, name: 'in_table', skipNormalization: true })
        );
    ...
    it('should ensure bookmark count equality after validation', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        para.addText(`Para ${i}`);
        para.addBookmarkStart(
          new Bookmark({ id: i + 100, name: `bm_${i}`, skipNormalization: true })
        );
      ...
      balPara.addBookmarkStart(
        new Bookmark({ id: 200, name: 'balanced', skipNormalization: true })
      );
