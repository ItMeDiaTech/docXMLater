...
- **`revisionHandling: 'reject'` reverts a loaded document to its original pre-edit state.** Loading with `Document.load(path, { revisionHandling: 'reject' })` is the exact inverse of the default `accept`: it removes inserted content (`w:ins`), restores deleted content (`w:del`, converting `w:delText` back to live `w:t`), undoes moves (keeps the `w:moveFrom` source, drops the `w:moveTo` destination), and rolls back tracked formatting changes by restoring the previous properties embedded in `w:rPrChange` / `w:pPrChange` / `w:tblPrChange` / `w:tcPrChange` / `w:trPrChange` / `w:sectPrChange` / `w:tblGridChange` / `w:tblPrExChange` markers. Inserted rows and tables are removed; deleted rows and tables are kept. The rejected document is fully editable and round-trips cleanly. Processing is DOM-based across `document.xml`, headers, footers, footnotes, endnotes, and comments, and revision metadata (`people.xml`, track-changes settings, revision count) is cleaned up as with `accept`. Consistent with `accept`, cell-level tracked markers (`w:cellIns`/`w:cellDel`/`w:cellMerge`) and the legacy `w:numberingChange` are not reverted (the latter records no recoverable previous value).
